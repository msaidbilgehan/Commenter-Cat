...
section: scope
in_scope:
  - "Rust orchestrator (cf-core + cf-engine + cf-cli workspace) owning the full substrate: walk, tree-sitter extract, kind classification, block coalescing, comment→code mapping, marker extraction, git enrichment, blame-skew rot candidates"
  ...
  - "Manifest-first provider layer: RuleProvider trait + invocation contract, run-state machine, JSONPath ManifestProvider (Tier 1) + generic SARIF ingest, declarative severity/category/capabilities maps, dogfooded built-in manifests (ruff/shellcheck/gitleaks), eslint Tier-2 native + Node tiers, pinning + config-fingerprint + cf doctor"
  - "Operations: cf check orchestration + native marker triage, parse-invariant applier, write-protection by kind, filter-up suppression + committed baseline + cf fix/tighten + opt-in native-directive export"
...
constraints:
  ...
  - "Repo tool-config is authoritative — CF layers rule-selection + suppression, never overrides a tool's config"
  ...
  - "Independently-versioned contracts (binary · MCP stability tiers · JSONL schema_version · cf_ruleset_version · manifest_version · config version); cache rebuilds, never migrates; provider bumps are comparability events, not CF breaking changes"
...
- **Architecture / authority:** repo tool-config wins (CF layers, never overrides); engine is the sole writer; content-hash cache keys; independently-versioned contracts with rebuild-over-migrate; max-accuracy with graceful degradation.
- **Reproducibility:** providers pinned + auto-managed by default (`--system-tools` escape hatch), comparability key `(cf_ruleset_version, provider_version, config_hash)`, byte-identical reports across runs and the OS matrix.
