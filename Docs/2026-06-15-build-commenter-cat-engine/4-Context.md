...
patterns:
  ...
  - "Target layout: Cargo workspace with commenter-cat-core (substrate types), commenter-cat-engine (orchestration library + MCP/render/storage/provider/ops), commenter-cat-cli (the `commenter-cat` binary)"
  - "MCP verbs are 1:1 with CLI verbs — both share one canonical verb surface on the commenter-cat-engine library (Idea §4a)"
  ...
  - "Layer discipline: domain code (commenter-cat-core) never imports infrastructure (rusqlite/subprocess/git); adapters translate at the seams"
reusable:
...
relevant_rules:
  - { path: "~/Workspace/GPT-Prompts/.claude/rules/general.md", focus: "ERR_SWALLOWED, ERR_BARE_RAISE, ERR_NO_CONTEXT (cause-chained context-rich errors); SEC_HARDCODED_SECRET / SEC_SENSITIVE_LOG (gitleaks finds, Commenter-Cat never stores; issue-tracker auth via host creds); SEC_COMMAND_INJECTION (subprocess arg arrays, never shell=true for provider spawns); PERF_UNBOUNDED_COLLECTION (bounded worker/subprocess pools, memory O(workers)); RES_LEAK (RAII/Drop for connections + subprocesses); ARCH_LAYER_VIOLATION (domain commenter-cat-core free of infra imports); ORG_MAGIC_NUMBER (τ≈0.8, severity tables, ranking weights as named consts); CONC_RACE (engine sole writer + WAL); API_BREAKING_CHANGE (independently-versioned contracts, MCP stability tiers)" }
...
## Patterns to Follow
- **Three-crate workspace:** `commenter-cat-core` (pure substrate types: error, version, config, finding, identity) · `commenter-cat-engine` (orchestration: walk, extract, map, git, storage, provider, ops, render, mcp, surface, ci, issues, hooks) · `commenter-cat-cli` (the `commenter-cat` binary). MCP and CLI share the engine library so verbs stay 1:1.
...
- **Delegate, don't reimplement:** rule content comes from ruff/eslint/shellcheck/gitleaks via `RuleProvider` adapters; built-ins ship as manifests (dogfooding); `commenter-cat` only spawns JSON-mode subprocesses and normalizes.
- **Layer discipline:** `commenter-cat-core` (domain) imports no infrastructure; subprocess/SQLite/git errors translate to `CommenterCatError` at the `commenter-cat-engine` adapter boundary.
...
graph TB
  ...
  subgraph core["commenter-cat-core (domain types)"]
