...
tasks:
  - id: "10.1"
    ...
    files: [crates/cf-engine/tests/prop_parse_invariance.rs, crates/cf-engine/tests/prop_identity.rs, crates/cf-core/tests/prop_coordinates.rs]
    depends_on: []
    parallel_safe: true
    validation: "cargo test -p cf-engine --test prop_parse_invariance --test prop_identity && cargo test -p cf-core --test prop_coordinates all pass"
  ...
  - id: "10.2"
    ...
    files: [crates/cf-engine/tests/golden_extraction.rs, crates/cf-engine/tests/provider_contract.rs, crates/cf-engine/tests/fixtures/README.md]
    depends_on: ["10.1"]
    parallel_safe: false
    validation: "cargo test -p cf-engine --test golden_extraction --test provider_contract pass; every built-in provider passes the shared contract harness"
  ...
  - id: "10.3"
    ...
    action: "Create integration tests on fixture repos with pinned providers (end-to-end cf check: unified findings, suppression, baseline, and the graceful-degradation path provider-absent→SKIPPED explicitly tested) and MCP agent-contract tests (token budget honored via limit/cursor/truncated, the apply_edit round-trip returns re-checked findings, write-protection-by-kind refusal); mock only at seams (network/clock) — never the parser or DB."
    files: [crates/cf-engine/tests/integration_check.rs, crates/cf-engine/tests/mcp_contract.rs, crates/cf-engine/tests/fixtures/repos/.gitkeep]
    depends_on: ["10.2"]
    parallel_safe: false
    validation: "cargo test -p cf-engine --test integration_check --test mcp_contract pass, including the provider-absent SKIPPED degradation path and the MCP token-budget + round-trip + write-protection assertions"
...
- **Files:** `crates/cf-engine/tests/{prop_parse_invariance.rs,prop_identity.rs}`, `crates/cf-core/tests/prop_coordinates.rs`
...
- **Files:** `crates/cf-engine/tests/{golden_extraction.rs,provider_contract.rs}`, `crates/cf-engine/tests/fixtures/README.md`
- **Depends on:** 10.1
...
- **Files:** `crates/cf-engine/tests/{integration_check.rs,mcp_contract.rs}`, `crates/cf-engine/tests/fixtures/repos/.gitkeep`
- **Depends on:** 10.2
...
- **Files:** `.github/workflows/{ci.yml,release.yml}`, `dist-workspace.toml`, `crates/cf-engine/benches/native_pass.rs`
- **Depends on:** 10.3
