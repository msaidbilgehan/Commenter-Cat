...
tasks:
  - id: "6.1"
    ...
    action: "Create the RuleProvider trait and invocation contract in cf-engine per Idea §5: one subprocess per provider batched over the cache-miss file set, parallel across providers, per-provider timeout, JSON-only I/O, file vs project scope, two-layer file discovery effective_scope = provider_filter(cf_scope), and canonical finding order (file, line, col, provider_rule_id)."
    files: [crates/cf-engine/src/provider/mod.rs, crates/cf-engine/src/provider/contract.rs, crates/cf-engine/src/provider/discovery.rs]
    depends_on: []
    parallel_safe: true
    validation: "cargo test -p cf-engine provider::contract passes: cf_scope authority (a cf-excluded file is never passed) and provider veto (a tool-config-ignored file is skipped) both hold"
    notes: "Idea §5 invocation contract + file discovery. cf_scope authoritative; provider only vetoes within it. JSON mode only — never text scraping. Trait is the seam mocked in tests, never the parser/DB (§11)."
  - id: "6.2"
    ...
    files: [crates/cf-engine/src/provider/run_state.rs, crates/cf-engine/src/provider/mod.rs]
    depends_on: ["6.1"]
    parallel_safe: false
    validation: "cargo test -p cf-engine provider::run_state passes: malformed JSON → PARTIAL (not EMPTY), absent binary → SKIPPED, nonzero-exit-with-valid-JSON → SUCCESS"
  ...
  - id: "6.3"
    ...
    files: [crates/cf-engine/src/provider/manifest/mod.rs, crates/cf-engine/src/provider/manifest/jsonpath.rs, crates/cf-engine/src/provider/manifest/sarif.rs]
    depends_on: ["6.1"]
    parallel_safe: true
    validation: "cargo test -p cf-engine provider::manifest passes: nested JSONPath extraction yields correct Findings, and a SARIF document maps via the generic mapper with no field-paths"
  ...
  - id: "6.4"
    ...
    files: [crates/cf-engine/src/provider/manifest/mapping.rs, crates/cf-engine/src/provider/manifest/capabilities.rs]
    depends_on: ["6.3"]
    parallel_safe: false
    validation: "cargo test -p cf-engine provider::manifest::mapping passes: severity_map/category_map produce canonical severity+category, and [capabilities] coordinate_system routes to the correct conversion from Phase 3"
  ...
  - id: "6.5"
    ...
    files: [crates/cf-engine/assets/providers/ruff.manifest.toml, crates/cf-engine/assets/providers/shellcheck.manifest.toml, crates/cf-engine/assets/providers/gitleaks.manifest.toml, crates/cf-engine/src/provider/builtins.rs]
    depends_on: ["6.4"]
    parallel_safe: false
    validation: "cargo test -p cf-engine provider::builtins passes: each built-in manifest loads via ManifestProvider and a recorded fixture (ruff/shellcheck/gitleaks JSON) maps to expected Findings"
  ...
  - id: "6.6"
    ...
    files: [crates/cf-engine/src/provider/native/eslint.rs, crates/cf-engine/src/provider/native/node_runtime.rs, crates/cf-engine/src/provider/native/mod.rs]
    depends_on: ["6.4"]
    parallel_safe: true
    validation: "cargo test -p cf-engine provider::native::eslint passes against a recorded eslint JSON fixture: jsdoc findings map with UTF-16→byte columns corrected; node_runtime reports the correct reproducibility_level"
  ...
  - id: "6.7"
    ...
    action: "Create provider pinning + reproducibility + doctor: fetch/cache pinned provider versions at ~/.cache/cf/providers/ (single-binary tools cleanly; eslint pins a full lockfile tree), pin by version/hash, record per-provider version + config_hash (over resolved effective config via eslint --print-config / ruff --show-settings / tsc --showConfig) in provider_state, default pinned mode with --system-tools escape hatch, and `cf doctor`/`cf doctor --providers` validating version AND config against the baseline."
    files: [crates/cf-engine/src/provider/management/mod.rs, crates/cf-engine/src/provider/management/pinning.rs, crates/cf-engine/src/provider/management/config_fingerprint.rs, crates/cf-engine/src/provider/management/doctor.rs]
    depends_on: ["6.5", "6.6"]
    parallel_safe: false
    validation: "cargo test -p cf-engine provider::management passes: config_hash over a resolved effective config detects a settings change, and doctor flags a version skew + config-differs while honoring pinned"
    notes: "Idea §5 Provider management & reproducibility: comparability key (cf_ruleset_version, provider_version, config_hash). Repo owns external-linter config; CF owns native-provider config. Baselines updated only under pinned tools. Depends on both manifest (6.5) and eslint (6.6) being present."
---

# Phase 6: Providers

## Goal
Build the manifest-first provider layer that makes Commenter-Cat a platform rather than a fixed bundle of four analyzers. The `RuleProvider` trait + invocation contract define how `cf` drives any tool; the run-state machine guarantees "provider failure ≠ zero findings"; the JSONPath `ManifestProvider` (Tier 1) covers ~80–90% of integrations declaratively; the dogfooded built-in manifests prove the design; the eslint native provider (Tier 2) is the lone documented exception with its Node-runtime tiers; and provider management delivers pinning, config-fingerprinting, and `cf doctor`. Depends on Phase 3 (the `Finding` schema + coordinate conversion) and Phase 4 (the provider-result cache). Runs in parallel with Phase 5 (identity).

## Tasks

### 6.1 — define-ruleprovider-trait
- **Action:** `RuleProvider` trait + invocation contract (subprocess, batched, parallel, timeout, JSON-only, scope, `provider_filter(cf_scope)`, canonical order).
- **Files:** `crates/cf-engine/src/provider/{mod.rs,contract.rs,discovery.rs}`
- **Depends on:** none (within phase)
- **Validation:** cf_scope authority + provider veto both hold.
...
- **Files:** `crates/cf-engine/src/provider/manifest/{mod.rs,jsonpath.rs,sarif.rs}`
...
- **Files:** `crates/cf-engine/src/provider/manifest/{mapping.rs,capabilities.rs}`
- **Depends on:** 6.3
...
- **Files:** `crates/cf-engine/assets/providers/{ruff,shellcheck,gitleaks}.manifest.toml`, `crates/cf-engine/src/provider/builtins.rs`
- **Depends on:** 6.4
...
- **Files:** `crates/cf-engine/src/provider/native/{eslint.rs,node_runtime.rs,mod.rs}`
...
- **Action:** Pinning + fetch/cache + config-fingerprint + `cf doctor`; pinned default, `--system-tools` escape hatch.
- **Files:** `crates/cf-engine/src/provider/management/{mod.rs,pinning.rs,config_fingerprint.rs,doctor.rs}`
- **Depends on:** 6.5, 6.6
- **Validation:** config_hash detects settings change; doctor flags version skew + config-differs under pinned.

## Phase Validation
`cargo test -p cf-engine provider::` passes across contract, run_state, manifest, builtins, native::eslint, and management; the provider layer can run the four built-in tools (three via dogfooded manifests, eslint native) over a fixture tree in JSON-only mode, normalize their output to canonical `Finding`s, resolve run-state correctly when a provider is absent or crashes, and report a coherent `cf doctor` version+config comparison.
