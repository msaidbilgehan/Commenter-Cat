...
tasks:
  - id: "2.1"
    ...
    action: "Create the file-walk module in commenter-cat-engine using the `ignore` crate: respect .gitignore/.ignore with an opt-out flag, apply [scan] include/exclude as the authoritative commenter_cat_scope, and add the generated-file header-sniff heuristic so minified/generated pseudo-comments are excluded."
    files: [crates/commenter-cat-engine/src/walk.rs, crates/commenter-cat-engine/src/lib.rs]
    depends_on: []
    parallel_safe: true
    validation: "cargo test -p commenter-cat-engine walk:: passes, covering gitignore respect, opt-out, and generated-file rejection on a fixture tree"
    notes: "Idea §3 + §5: commenter_cat_scope is authoritative; providers later veto within it (provider_filter(commenter_cat_scope)). The `ignore` crate is ripgrep's engine (§10)."
  - id: "2.2"
    ...
    files: [crates/commenter-cat-engine/src/extract/mod.rs, crates/commenter-cat-engine/src/extract/kind.rs, crates/commenter-cat-engine/src/extract/grammars.rs]
    depends_on: ["2.1"]
    parallel_safe: false
    validation: "cargo test -p commenter-cat-engine extract:: passes; an `insta` snapshot per grammar captures kind + ranges with zero string-vs-comment false positives"
    notes: "Idea §3: tree-sitter not regex. `directive` = commenter-cat:* control comments (never finding targets). shebang+license default-suppressed. CRLF byte-offset handling tested here (§10/§11)."
  - id: "2.3"
    ...
    files: [crates/commenter-cat-engine/src/extract/coalesce.rs, crates/commenter-cat-engine/src/extract/mod.rs]
    depends_on: ["2.2"]
    parallel_safe: false
    validation: "cargo test -p commenter-cat-engine extract::coalesce passes on a fixture with adjacent line comments + an interrupting blank line"
  ...
  - id: "2.4"
    ...
    files: [crates/commenter-cat-engine/src/map/mod.rs, crates/commenter-cat-engine/src/map/doc_binding.rs, crates/commenter-cat-engine/src/map/geometry.rs]
    depends_on: ["2.3"]
    parallel_safe: false
    validation: "cargo test -p commenter-cat-engine map:: passes; `insta` snapshots verify bound_symbol/bound_node_range for doc, trailing, lead, and orphan cases per language"
  ...
  - id: "2.5"
    ...
    files: [crates/commenter-cat-engine/src/markers.rs, crates/commenter-cat-engine/src/lib.rs]
    depends_on: ["2.2"]
    parallel_safe: true
    validation: "cargo test -p commenter-cat-engine markers:: passes, covering builtin + custom markers and a TODO→FIXME distinction"
  ...
  - id: "2.6"
    ...
    files: [crates/commenter-cat-engine/src/git/mod.rs, crates/commenter-cat-engine/src/git/blame.rs, crates/commenter-cat-engine/src/git/repo.rs]
    depends_on: ["2.4"]
    parallel_safe: false
    validation: "cargo test -p commenter-cat-engine git:: passes against a fixture repo with known blame history"
  ...
  - id: "2.7"
    ...
    files: [crates/commenter-cat-engine/src/rot.rs, crates/commenter-cat-engine/src/lib.rs]
    depends_on: ["2.6"]
    parallel_safe: false
    validation: "cargo test -p commenter-cat-engine rot:: passes on a fixture where comment-blame predates code-blame and a control where it does not"
...
- **Action:** File walk via the `ignore` crate with gitignore respect, opt-out, authoritative commenter_cat_scope, and generated-file header sniff.
- **Files:** `crates/commenter-cat-engine/src/walk.rs`, `crates/commenter-cat-engine/src/lib.rs`
- **Depends on:** none (within phase; phase depends on P1)
- **Validation:** `cargo test -p commenter-cat-engine walk::` passes (gitignore, opt-out, generated rejection).
...
- **Files:** `crates/commenter-cat-engine/src/extract/{mod.rs,kind.rs,grammars.rs}`
- **Depends on:** 2.1
- **Validation:** `cargo test -p commenter-cat-engine extract::` + per-grammar `insta` snapshots; zero false positives.
...
- **Validation:** `cargo test -p commenter-cat-engine extract::coalesce` on adjacent-with-blank-line fixture.
...
- **Files:** `crates/commenter-cat-engine/src/map/{mod.rs,doc_binding.rs,geometry.rs}`
- **Depends on:** 2.3
- **Validation:** `cargo test -p commenter-cat-engine map::` + `insta` snapshots for doc/trailing/lead/orphan per language.
...
- **Validation:** `cargo test -p commenter-cat-engine markers::` (builtin + custom + TODO/FIXME distinction).

### 2.6 — implement-git-enrichment
- **Action:** `gix`-based repo-root resolution, blame join (author/date/commit), and changed-file sets.
- **Files:** `crates/commenter-cat-engine/src/git/{mod.rs,blame.rs,repo.rs}`
- **Depends on:** 2.4
- **Validation:** `cargo test -p commenter-cat-engine git::` against a fixture repo with known history.
...
- **Files:** `crates/commenter-cat-engine/src/rot.rs`, `crates/commenter-cat-engine/src/lib.rs`
- **Depends on:** 2.6
- **Validation:** `cargo test -p commenter-cat-engine rot::` on skewed + control fixtures.

## Phase Validation
`cargo test -p commenter-cat-engine` passes for walk, extract, map, markers, git, and rot modules; the native pass produces a complete comment record (path, hashes, kind, ranges, raw text, bound_symbol, bound_node_range, marker tags, git author/date/commit, is_rot_candidate) for a multi-language fixture tree — every field of the Idea §4 "Comment record" shape except `findings[]` (added in Phase 3+).
