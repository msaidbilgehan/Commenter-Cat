...
tasks:
  - id: "4.1"
    ...
    action: "Create the inputs.db layer in commenter-cat-engine via rusqlite (bundled SQLite, WAL mode, load_extension enabled): a content-addressed store keyed (content_hash, provider, provider_version) for provider-result JSON and (content_hash, model_version) for embedding vectors, with INPUTS_DB_SCHEMA_VERSION recorded in a meta table; engine is the sole writer with busy_timeout."
    files: [crates/commenter-cat-engine/src/storage/mod.rs, crates/commenter-cat-engine/src/storage/inputs_db.rs, crates/commenter-cat-engine/src/storage/connection.rs]
    depends_on: []
    parallel_safe: true
    validation: "cargo test -p commenter-cat-engine storage::inputs_db passes: insert+lookup of a provider result by (content_hash, provider, version) and survival across a simulated index-schema bump"
  ...
  - id: "4.2"
    ...
    action: "Create the index.db layer: relational comment-facts tables (the Idea §4 comment record fields), the mapping table, identity/suppression tables (populated in Phase 5/7), and an INDEX_DB_SCHEMA_VERSION meta row; this is the derived, cheap-to-rebuild, Commenter-Cat-schema-versioned layer."
    files: [crates/commenter-cat-engine/src/storage/index_db.rs, crates/commenter-cat-engine/src/storage/schema.rs]
    depends_on: ["4.1"]
    parallel_safe: false
    validation: "cargo test -p commenter-cat-engine storage::index_db passes: comment-fact upsert + read round-trips a full §4 comment record (sans search indexes)"
  ...
  - id: "4.3"
    ...
    files: [crates/commenter-cat-engine/src/storage/cache.rs, crates/commenter-cat-engine/src/storage/hashing.rs]
    depends_on: ["4.1"]
    parallel_safe: true
    validation: "cargo test -p commenter-cat-engine storage::cache passes: a changed byte invalidates the entry, an unchanged file yields a cache hit, and an mtime-only change does not invalidate"
  ...
  - id: "4.4"
    ...
    files: [crates/commenter-cat-engine/src/storage/fts.rs, crates/commenter-cat-engine/src/storage/index_db.rs]
    depends_on: ["4.2"]
    parallel_safe: false
    validation: "cargo test -p commenter-cat-engine storage::fts passes: a keyword query returns the expected comments ranked"
  ...
  - id: "4.5"
    ...
    files: [crates/commenter-cat-engine/src/embed/mod.rs, crates/commenter-cat-engine/src/embed/onnx.rs]
    depends_on: ["4.1"]
    parallel_safe: true
    validation: "cargo test -p commenter-cat-engine embed:: passes: a deterministic vector is produced for fixed text and persisted/retrieved by (content_hash, model_version)"
  ...
  - id: "4.6"
    ...
    files: [crates/commenter-cat-engine/src/storage/vec_index.rs, crates/commenter-cat-engine/src/storage/connection.rs]
    depends_on: ["4.5", "4.2"]
    parallel_safe: false
    validation: "cargo test -p commenter-cat-engine storage::vec_index passes: kNN over inserted vectors returns nearest first; extension loads on the host platform"
  ...
  - id: "4.7"
    ...
    files: [crates/commenter-cat-engine/src/search/mod.rs, crates/commenter-cat-engine/src/search/rrf.rs]
    depends_on: ["4.4", "4.6"]
    parallel_safe: false
    validation: "cargo test -p commenter-cat-engine search:: passes: RRF fuses keyword+semantic rankings with the documented tie-break"
  ...
  - id: "4.8"
    ...
    files: [crates/commenter-cat-engine/src/storage/rebuild.rs, crates/commenter-cat-engine/src/storage/location.rs]
    depends_on: ["4.7", "4.3"]
    parallel_safe: false
    validation: "cargo test -p commenter-cat-engine storage::rebuild passes: an index-schema bump triggers a byte-identical re-derive from inputs.db with zero provider invocations"
...
- **Files:** `crates/commenter-cat-engine/src/storage/{mod.rs,inputs_db.rs,connection.rs}`
...
- **Files:** `crates/commenter-cat-engine/src/storage/{index_db.rs,schema.rs}`
- **Depends on:** 4.1
...
- **Files:** `crates/commenter-cat-engine/src/storage/{cache.rs,hashing.rs}`
...
- **Files:** `crates/commenter-cat-engine/src/embed/{mod.rs,onnx.rs}`
...
- **Files:** `crates/commenter-cat-engine/src/search/{mod.rs,rrf.rs}`
- **Depends on:** 4.4, 4.6
