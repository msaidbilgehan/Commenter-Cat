...
tasks:
  - id: "3.1"
    ...
    action: "Create the canonical Finding struct in cf-core per Idea §5: fields origin (enum ruff|eslint|shellcheck|gitleaks|native), provider_rule_id (lossless origin-qualified), canonical_rule_id, category (enum: doc_missing, doc_drift, doc_style, commented_code, secret, todo_format, marker_stale, comment_style, shebang, directive, rot_candidate), severity, severity_native, message, target (comment_id|bound_symbol), range, fix (provider_autofix|agent_only|none), url; plus the Category enum and a FindingTarget enum."
    files: [crates/cf-core/src/finding/mod.rs, crates/cf-core/src/finding/category.rs, crates/cf-core/src/lib.rs]
    depends_on: []
    parallel_safe: true
    validation: "cargo test -p cf-core finding:: passes; serde round-trip of a Finding is byte-stable"
  ...
  - id: "3.2"
    ...
    files: [crates/cf-core/src/finding/severity.rs, crates/cf-core/src/finding/mod.rs]
    depends_on: ["3.1"]
    parallel_safe: false
    validation: "cargo test -p cf-core finding::severity passes: config override beats category, category beats per-tool, and doc_drift resolves to error from both ruff and eslint origins"
  ...
  - id: "3.3"
    ...
    files: [crates/cf-core/src/finding/coordinates.rs, crates/cf-core/src/finding/mod.rs]
    depends_on: ["3.1"]
    parallel_safe: false
    validation: "cargo test -p cf-core finding::coordinates passes, including a UTF-16 column with a multi-byte char before it and a 0-based↔1-based case"
  ...
  - id: "3.4"
    ...
    files: [crates/cf-core/src/finding/dedup.rs, crates/cf-core/src/finding/mod.rs]
    depends_on: ["3.2", "3.3"]
    parallel_safe: false
    validation: "cargo test -p cf-core finding::dedup passes: overlapping same-category findings collapse to one with unioned origins and max severity, and output order is canonical"
...
## Goal
Define the one schema every provider adapter and every native check reconciles to: the canonical `Finding`, the category-anchored 4-level severity resolution, the coordinate-conversion layer (the eslint UTF-16 trap), and the same-category dedup with canonical ordering. This phase lives in `cf-core` and depends only on Phase 1, so it runs in parallel with Phase 2 (native substrate). It is a hard prerequisite for both Phase 4 storage (which persists findings) and Phase 6 providers (which produce them).

## Tasks

### 3.1 — define-finding-struct
- **Action:** Canonical `Finding` struct + `Category`/`FindingTarget` enums per Idea §5.
- **Files:** `crates/cf-core/src/finding/{mod.rs,category.rs}`, `crates/cf-core/src/lib.rs`
- **Depends on:** none (within phase; phase depends on P1)
- **Validation:** `cargo test -p cf-core finding::`; serde round-trip byte-stable.
...
## Phase Validation
`cargo test -p cf-core finding::` passes end-to-end: a mixed list of provider-shaped and native-shaped findings can be normalized into canonical `Finding`s, severity-resolved, coordinate-converted, deduped, and emitted in canonical order deterministically across two runs.
