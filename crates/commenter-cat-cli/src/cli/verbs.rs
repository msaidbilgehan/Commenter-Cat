...
use commenter_cat_engine::ops::check::CheckResult;
use commenter_cat_engine::ops::index::{self, Session};
use commenter_cat_engine::ops::{self};
use commenter_cat_engine::provider::{builtins, RuleProvider, RunState};
use commenter_cat_engine::render;
use commenter_cat_engine::surface::{ranking, roundtrip, token_economy};
...
///
/// # Errors
/// Returns [`CommenterCatError`] on any unrecoverable failure; `main` renders the cause
/// chain and exits non-zero.
pub(crate) fn run(cli: Cli) -> CommenterCatResult<i32> {
...
/// `commenter-cat mcp` — serve the MCP protocol over stdio (the agent-facing product).
fn run_mcp() -> CommenterCatResult<i32> {
    commenter_cat_engine::mcp::server::serve_stdio()?;
    Ok(render::EXIT_OK)
}

/// `commenter-cat install-hooks` — install the non-fatal cache-warmer hooks via
/// `core.hooksPath` (Idea §7).
fn run_install_hooks() -> CommenterCatResult<i32> {
    let root = root_for(&[])?;
    let report = commenter_cat_engine::hooks::install(&root)?;
    print(&format!(
...
/// `commenter-cat check` — the native pass + built-in providers, rendered, with a CI verdict.
fn run_check(
    paths: &[PathBuf],
    format: commenter_cat_core::config::OutputFormat,
    strict: bool,
    use_cache: bool,
    stats: bool,
    ...
    // Suppressed findings (inline `commenter-cat:*` directives + the committed baseline) are
...
/// the CI verdict gates on, so a `commenter-cat:disable` / baselined finding never fails CI.
...
fn report_suppressions(result: &CheckResult, show_suppressed: bool) {
    ...
    if show_suppressed {
        eprintln!("commenter-cat: note: {suppressed} suppressed finding(s) shown (audit view).");
    } else {
        eprintln!(
            "commenter-cat: note: {suppressed} finding(s) suppressed by directives/baseline \
...
///
/// `commenter-cat check` reports findings on stdout, but a provider that crashed
...
fn report_diagnostics(result: &CheckResult) {
    for (provider, state) in &result.run_states {
        match state {
            RunState::Partial => eprintln!(
                "commenter-cat: warning: provider {provider:?} PARTIAL — it ran but its output was unusable; \
                 its findings are UNAVAILABLE (not zero). Re-run with --strict to fail the build."
            ),
            RunState::Skipped => eprintln!(
                "commenter-cat: note: provider {provider:?} skipped (not installed or unavailable) — \
    ...
    if !result.unattached.is_empty() {
        eprintln!(
            "commenter-cat: note: {} provider finding(s) target a symbol with no comment \
        ...
        for (rule, count) in counts.iter().take(MAX_UNATTACHED_RULES_SHOWN) {
            eprintln!("commenter-cat:   {rule}: {count}");
        }
    }
}
...
fn report_stats(result: &CheckResult, persist: Duration, total: Duration) {
    ...
    eprintln!(
        "commenter-cat: stats: walk {}ms · native {}ms · providers {}ms · fuse {}ms · index {}ms · total {}ms",
        t.walk.as_millis(),
    ...
    eprintln!(
        "commenter-cat: stats: provider cache — {} served from cache, {} ran",
        cache.hits, cache.runs
    );
}

/// `commenter-cat candidates` — the native worklist (rot + markers), ranked and bounded.
fn run_candidates(limit: usize) -> CommenterCatResult<i32> {
    let root = root_for(&[])?;
    let config = config::discover(&root)?;
...
/// `commenter-cat doctor` — list the providers `commenter-cat` would run and their declared contract.
fn run_doctor() -> CommenterCatResult<i32> {
    let providers = builtins::load_all()?;
...
/// `commenter-cat query` — FIND: search the persisted index, ranked + bounded.
fn run_query(query: &str, limit: usize, cursor: Option<usize>) -> CommenterCatResult<i32> {
...
/// `commenter-cat context` — UNDERSTAND: one comment, with its bound code on request.
fn run_context(comment_id: &str, with_code: bool) -> CommenterCatResult<i32> {
    let session = Session::open(&root_for(&[])?)?;
...
/// `commenter-cat apply-edit` — UPDATE: a parse-invariant comment edit, written to disk,
...
/// `commenter-cat remove` — UPDATE: remove a comment, written to disk, then re-checked.
fn run_remove(comment_id: &str, allow_significant: bool) -> CommenterCatResult<i32> {
    let session = Session::open(&root_for(&[])?)?;
    let comment = resolve_comment(&session, comment_id)?;
    let source = session.read_source(&comment)?;
    let config = config::discover(session.repo_root())?;
...
fn resolve_comment(session: &Session, comment_id: &str) -> CommenterCatResult<Comment> {
    let id: i64 = comment_id.parse().map_err(|_| {
        CommenterCatError::config(format!(
            "invalid comment id {comment_id:?} (expected a number from `commenter-cat query`)"
        ))
    })?;
    session.comment(id)?.ok_or_else(|| {
        CommenterCatError::config(format!(
            "no comment {id} in the index (re-run `commenter-cat check`?)"
        ))
    })
}
...
fn edit_report(
    verb: &str,
    comment: &Comment,
    findings: &[commenter_cat_core::finding::Finding],
) -> String {
    let mut out = format!(
        "{verb} comment at {}:{}.\n",
...
impl NativeCandidate {
    fn new(
        file: &str,
        finding: &commenter_cat_core::finding::Finding,
        comment: &commenter_cat_core::comment::Comment,
    ) -> Self {
...
fn root_for(paths: &[PathBuf]) -> CommenterCatResult<PathBuf> {
    match paths.first() {
...
/// `commenter-cat baseline accept|prune` — manage the committed `commenter-cat.baseline.toml`.
///
...
fn run_baseline(action: &BaselineAction, use_cache: bool) -> CommenterCatResult<i32> {
    let root = root_for(&[])?;
    let config = config::discover(&root)?;
    let providers = builtins::load_all()?;
    let provider_refs: Vec<&dyn RuleProvider> = providers
        ...
        BaselineAction::Prune => {
            if !path.exists() {
                return Err(CommenterCatError::config(format!(
                    "no baseline at {} to prune — run `commenter-cat baseline accept` first",
                    path.display()
                )));
            }
...
/// `commenter-cat suppressions export` — materialize Commenter-Cat's suppression set into each tool's
...
fn run_suppressions(action: &SuppressionsAction, use_cache: bool) -> CommenterCatResult<i32> {
    match action {
        SuppressionsAction::Export => {
            ...
            if report.already_present > 0 {
                eprintln!(
                    "commenter-cat: note: {} finding(s) already carried their native directive (skipped).",
                    report.already_present
                );
            }
            if report.no_native_directive > 0 {
                eprintln!(
                    "commenter-cat: note: {} suppressed finding(s) are Commenter-Cat-native — no tool directive to export.",
...
/// `commenter-cat issues sync` — bidirectional comment-to-issue sync (Idea §9), idempotent
...
fn run_issues(action: &IssuesAction, use_cache: bool) -> CommenterCatResult<i32> {
    match action {
        IssuesAction::Sync { apply } => run_issues_sync(*apply, use_cache),
    }
}

fn run_issues_sync(apply: bool, use_cache: bool) -> CommenterCatResult<i32> {
    let root = root_for(&[])?;
    let config = config::discover(&root)?;
    let providers = builtins::load_all()?;
    let provider_refs: Vec<&dyn RuleProvider> = providers
    ...
    let mut out = if apply {
        ...
        format!(
            "Dry run: {} would be filed; {already} already tracked; {resolved} would be \
             resolved (comment removed). Re-run `commenter-cat issues sync --apply` to file + reconcile \
             (network + gh).\n",
            new_tokens.len()
        )
    };
    ...
    for skip in &reconcile.skipped {
        eprintln!("commenter-cat: note: issues sync — {skip}");
    }
    Ok(render::EXIT_OK)
}
...
mod tests {
    ...
    fn check_dir(dir: &Path, strict: bool) -> i32 {
        run_check(
            &[dir.to_path_buf()],
            commenter_cat_core::config::OutputFormat::Jsonl,
            strict,
            false,
            false,
            false,
    ...
    fn test_baseline_accept_snapshots_and_prune_keeps_live() {
        // `commenter-cat baseline accept` snapshots the current findings' Tier-2 identities;
        // pruning against the same findings keeps every entry (nothing is stale).
        let dir = seeded_repo();
        let config = commenter_cat_core::config::ResolvedConfig::default();
        let no_providers: [&dyn RuleProvider; 0] = [];
    ...
    fn test_check_persists_an_index_for_the_read_verbs() {
        // `commenter-cat check` must leave a populated index behind so query/context resolve.
        let dir = seeded_repo();
