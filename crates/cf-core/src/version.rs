...
//! | [`CF_RULESET_VERSION`] | canonical rule/category vocabulary | bump = a findings-comparability event (re-baseline), not a binary break |
//! | [`MANIFEST_VERSION`] | provider manifest format | CF reads current + prior major |
...
/// Version of CF's canonical rule/category vocabulary (Idea §5, §11).
///
/// A bump changes which rules CF ingests or how they map to canonical ids /
...
pub const CF_RULESET_VERSION: u32 = 1;

/// Version of the provider manifest format (Idea §5, §11).
///
/// CF reads the current and prior major so third-party adapters survive a CF
/// upgrade. Currently `1`.
...
/// The `cf` binary's SemVer, sourced from the crate manifest (Idea §11: 1.0+).
#[must_use]
pub fn cf_version() -> &'static str {
...
///
/// Per Idea §11, CF reads the current manifest major and the prior one, so an
/// adapter written for an older CF keeps working after an upgrade.
#[must_use]
...
/// comparable only when **all three** of these agree: a CF ruleset change, a
/// provider upgrade, or a resolved-config change each independently invalidates
/// comparability and requires a re-baseline (surfaced by `cf doctor`).
///
...
pub struct ComparabilityKey {
    /// CF's canonical-vocabulary version at the time of the run.
    pub cf_ruleset_version: u32,
...
impl ComparabilityKey {
    ...
    /// baseline artifacts produced by a different CF build).
    pub fn new(
        cf_ruleset_version: u32,
...
mod tests {
    ...
    fn test_comparability_key_equality() {
        ...
        let diff_ruleset = ComparabilityKey::new(CF_RULESET_VERSION + 1, "0.14.2", "sha256:abc");
    ...
    fn test_current_stamps_ruleset_version() {
        ...
        assert_eq!(key.cf_ruleset_version, CF_RULESET_VERSION);
    }
    ...
    #[test]
    fn test_cf_version_is_nonempty() {
        assert!(!cf_version().is_empty());
    }
}
