...
//! `commenter-cat check` fusion, filter-up suppression, the committed baseline diff, and the
...
use commenter_cat_core::finding::{Finding, Origin};
use commenter_cat_core::identity::fingerprint::cosmetic_fingerprint;
use commenter_cat_engine::ci::diff::new_findings;
use commenter_cat_engine::ops::baseline;
use commenter_cat_engine::ops::check::check;
use commenter_cat_engine::ops::suppress::{self, directives};
use commenter_cat_engine::provider::manifest::ManifestProvider;
use commenter_cat_engine::provider::{ProviderContext, RuleProvider, RunState};
...
fn directive_suppresses_a_matching_finding() {
    ...
    let directive = directives::parse(
        "# commenter-cat:disable-line=marker:TODO",
        todo.range.start_line,
    )
    .unwrap();
...
fn absent_provider_degrades_to_skipped_not_a_crash() {
    ...
    let manifest = "manifest_version = 1\n\
                    command = [\"commenter-cat-absent-tool-xyz123\", \"{files}\"]\n\
                    format = \"json\"\n\
                    scope = \"file\"\n\
                    [capabilities]\n\
