...
//!
//! `commenter-cat` is a **conductor**, not an analyzer: it drives best-in-class external
//! tools through swappable adapters behind one [`RuleProvider`] trait, then
//! normalizes their output into the canonical [`commenter_cat_core::Finding`]. The layer is
...
//! * [`discovery`] — `effective_scope = provider_filter(commenter_cat_scope)` (6.1).
