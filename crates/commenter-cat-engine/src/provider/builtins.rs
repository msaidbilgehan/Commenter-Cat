...
///
/// # Errors
/// Returns [`commenter_cat_core::CommenterCatError`] if the embedded manifest fails to parse.
pub fn load_builtin(id: &str) -> Option<CommenterCatResult<ManifestProvider>> {
    BUILTIN_MANIFESTS
        .iter()
        .find(|(name, _)| *name == id)
...
///
/// # Errors
/// Returns [`commenter_cat_core::CommenterCatError`] if any embedded manifest fails to parse.
pub fn load_all() -> CommenterCatResult<Vec<ManifestProvider>> {
    BUILTIN_MANIFESTS
        .iter()
        .map(|(name, toml)| ManifestProvider::from_toml(*name, toml))
        .collect()
