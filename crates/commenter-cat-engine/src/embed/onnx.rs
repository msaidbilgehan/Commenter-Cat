...
impl OnnxEmbedder {
    ...
    ///
    /// # Errors
    /// Returns [`CommenterCatError::Storage`] if the model cannot be fetched or loaded.
    pub fn new() -> CommenterCatResult<Self> {
    ...
    /// # Errors
    /// Returns [`CommenterCatError::Storage`] if the model cannot be fetched or loaded.
    pub fn with_cache_dir(cache_dir: PathBuf) -> CommenterCatResult<Self> {
        Self::load(
    ...
    fn load(options: InitOptions) -> CommenterCatResult<Self> {
        let model = TextEmbedding::try_new(options).map_err(|e| {
            CommenterCatError::storage(format!("loading ONNX embedding model: {e}"))
        })?;
        Ok(Self {
...
impl Embedder for OnnxEmbedder {
    ...
    fn embed(&self, text: &str) -> CommenterCatResult<Vec<f32>> {
        ...
        vectors
            .pop()
            .ok_or_else(|| CommenterCatError::storage("ONNX model returned no embedding"))
    }
    ...
    fn embed_batch(&self, texts: &[&str]) -> CommenterCatResult<Vec<Vec<f32>>> {
        let mut model = self
            .model
            .lock()
            .map_err(|_| CommenterCatError::storage("embedding model lock poisoned"))?;
        model
            .embed(texts, None)
            .map_err(|e| CommenterCatError::storage(format!("ONNX inference failed: {e}")))
    }
}
