...
//! applier (7.2) and the same fuse machinery as `commenter-cat check` (7.1), scoped to the
...
///
/// # Errors
/// Returns [`commenter_cat_core::CommenterCatError`] if write-protection refuses, parse-invariance is
...
///
/// # Errors
/// Returns [`commenter_cat_core::CommenterCatError`] under the same conditions as
...
mod tests {
    ...
    fn test_edit_introducing_a_todo_is_rechecked_inline() {
        let root = Path::new("/tmp/commenter-cat-roundtrip-test");
