...
///
/// # Errors
/// Returns [`CommenterCatError::Git`] if the file cannot be blamed (e.g. it is untracked
/// at HEAD) or a referenced commit cannot be read.
pub fn blame_file(repo: &Repo, repo_path: &str, head: ObjectId) -> CommenterCatResult<FileBlame> {
    let path = BStr::new(repo_path);
    let outcome = repo
        .inner()
        .blame_file(path, head, gix::repository::blame_file::Options::default())
        .map_err(|e| CommenterCatError::git(format!("blaming {repo_path}")).caused_by(e))?;
...
fn resolve_commit(repo: &Repo, commit_id: ObjectId) -> CommenterCatResult<LineBlame> {
    let commit = repo
        .inner()
        .find_commit(commit_id)
        .map_err(|e| CommenterCatError::git(format!("reading commit {commit_id}")).caused_by(e))?;
    let time = commit.time().map_err(|e| {
        CommenterCatError::git(format!("reading time of commit {commit_id}")).caused_by(e)
    })?;
    let author = commit.author().map_err(|e| {
        CommenterCatError::git(format!("reading author of commit {commit_id}")).caused_by(e)
    })?;
