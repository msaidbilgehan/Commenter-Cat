...
use commenter_cat_core::comment::{Comment, GitInfo};
...
///
/// # Errors
/// Returns [`CommenterCatError::Git`] only when HEAD cannot be resolved (no repository or
/// an empty one) — the caller treats that as "git unavailable" and proceeds.
pub fn enrich(repo: &Repo, comments: &mut [Comment]) -> CommenterCatResult<BlameIndex> {
