...
impl Repo {
    ...
    ///
    /// # Errors
    /// Returns [`CommenterCatError::Git`] if no repository is found or it is bare.
    pub fn discover(start: &Path) -> CommenterCatResult<Repo> {
        let inner = gix::discover(start).map_err(|e| {
            CommenterCatError::git(format!("discovering repository at {}", start.display()))
                .caused_by(e)
        })?;
        let workdir = inner
            .workdir()
            .ok_or_else(|| CommenterCatError::git("repository is bare (no work directory)"))?
            .to_path_buf();
    ...
    ///
    /// # Errors
    /// Returns [`CommenterCatError::Git`] if HEAD has no commit yet (an empty repository).
    pub fn head_commit_id(&self) -> CommenterCatResult<ObjectId> {
        let head = self
            .inner
            .head_id()
            .map_err(|e| CommenterCatError::git("resolving HEAD commit").caused_by(e))?;
        Ok(head.detach())
    }
}
