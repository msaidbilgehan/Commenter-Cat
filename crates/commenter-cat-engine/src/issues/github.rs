...
impl IssueBackend for GhCliBackend {
    fn create(&self, request: &IssueRequest) -> CommenterCatResult<IssueRef> {
        let output = Command::new("gh")
            ...
            .output()
            .map_err(|e| {
                CommenterCatError::provider("gh", "spawning `gh issue create`").caused_by(e)
            })?;
    ...
    fn get(&self, id: &str) -> CommenterCatResult<Option<IssueRef>> {
        let output = Command::new("gh")
            ...
            .output()
            .map_err(|e| {
                CommenterCatError::provider("gh", "spawning `gh issue view`").caused_by(e)
            })?;
        if !output.status.success() {
        ...
        let value: serde_json::Value = serde_json::from_slice(&output.stdout).map_err(|e| {
            CommenterCatError::provider("gh", "parsing `gh issue view` JSON").caused_by(e)
        })?;
        let url = value
            .get("url")
