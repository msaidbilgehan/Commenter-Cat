...
pub fn load(path: &Path) -> CommenterCatResult<IssueLedger> {
    let text = std::fs::read_to_string(path).map_err(|e| {
        CommenterCatError::config(format!("reading issue ledger {}", path.display())).caused_by(e)
    })?;
    let file: LedgerFile = toml::from_str(&text).map_err(|e| {
        CommenterCatError::config(format!("parsing issue ledger {}", path.display())).caused_by(e)
    })?;
    if file.version > LEDGER_VERSION {
        return Err(CommenterCatError::config(format!(
            "issue ledger at {} declares version {}, but this commenter-cat supports up to {LEDGER_VERSION}",
            path.display(),
            file.version
        )));
    }
...
pub fn save(ledger: &IssueLedger, path: &Path) -> CommenterCatResult<()> {
    ...
    let text = toml::to_string(&file)
        .map_err(|e| CommenterCatError::config("serializing issue ledger").caused_by(e))?;
    std::fs::write(path, text).map_err(|e| {
        CommenterCatError::config(format!("writing issue ledger {}", path.display())).caused_by(e)
    })?;
    Ok(())
}
