...
///
/// # Errors
/// Returns [`commenter_cat_core::CommenterCatError`] if the backend create call fails.
pub fn file_issue(
    token: &str,
    request: &IssueRequest,
    backend: &dyn IssueBackend,
    ledger: &mut IssueLedger,
) -> CommenterCatResult<IssueRef> {
...
///
/// # Errors
/// Returns [`commenter_cat_core::CommenterCatError`] if the backend lookup or the safe-write fails.
pub fn sync_resolution(
    ...
    backend: &dyn IssueBackend,
    ledger: &IssueLedger,
) -> CommenterCatResult<Option<ApplyResult>> {
...
mod tests {
    ...
    fn test_reworded_comment_does_not_double_file() {
        ...
        let mut original = Comment::new(
            ...
            "h1",
            Language::Python,
            commenter_cat_core::kind::CommentKind::Line,
            range(),
            "# TODO fix the parser",
        );
        original.bound_symbol = Some(commenter_cat_core::symbol::BoundSymbol::new("app.parse"));
    ...
    fn range() -> commenter_cat_core::finding::Range {
        commenter_cat_core::finding::Range::new(0, 16, 1, 1)
    }
}
