...
//!
//! `commenter-cat suppressions export` is the **opt-in, one-way inverse of filter-up**: it
...
fn directive_marker(origin: &Origin) -> Option<&'static str> {
    match origin {
        ...
        // Native Commenter-Cat findings and third-party providers have no native directive.
...
///
/// # Errors
/// Returns [`commenter_cat_core::CommenterCatError`] if the insertion would alter code.
...
/// The outcome of `commenter-cat suppressions export` — what was written and what was
...
pub struct ExportReport {
    ...
    /// Suppressed findings whose origin has no native directive (Commenter-Cat-native, etc.).
...
/// idempotently (a directive already present is left alone). Commenter-Cat-native findings
...
///
/// # Errors
/// Returns [`commenter_cat_core::CommenterCatError`] on a file read/write failure or if an insertion
/// would alter code (parse-invariance).
pub fn export_suppressions(
    root: &Path,
    comments: &[Comment],
    suppressed: &[SuppressedFinding],
) -> CommenterCatResult<ExportReport> {
    ...
    for (path, group) in by_file {
        ...
        let mut source = std::fs::read_to_string(&absolute).map_err(|e| {
            CommenterCatError::storage(format!("reading {}", absolute.display())).caused_by(e)
        })?;
...
mod tests {
    use super::*;
    use commenter_cat_core::finding::{Category, Finding, FindingTarget, Fix, Range};
    ...
    fn suppressed_first() -> Vec<SuppressedFinding> {
        vec![SuppressedFinding {
            comment_index: 0,
            finding_index: 0,
            suppressed_by: "commenter-cat:disable".to_owned(),
        }]
    }
