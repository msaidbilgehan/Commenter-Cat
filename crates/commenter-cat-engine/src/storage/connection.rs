...
//!
//! Every Commenter-Cat connection is opened through here so it gets the same setup: WAL
...
/// Opens (creating if needed) a Commenter-Cat database at `path` with the standard setup.
///
/// # Errors
/// Returns [`CommenterCatError::Storage`] if the database cannot be opened or configured.
pub fn open(path: &Path) -> CommenterCatResult<Connection> {
    register_sqlite_vec();
    let conn = Connection::open(path).map_err(|e| {
        CommenterCatError::storage(format!("opening database at {}", path.display())).caused_by(e)
    })?;
    configure(&conn)?;
    Ok(conn)
}

/// Opens an in-memory Commenter-Cat database (for tests and ephemeral derivations).
///
/// # Errors
/// Returns [`CommenterCatError::Storage`] if the database cannot be opened or configured.
pub fn open_in_memory() -> CommenterCatResult<Connection> {
    register_sqlite_vec();
    let conn = Connection::open_in_memory()
        .map_err(|e| CommenterCatError::storage("opening in-memory database").caused_by(e))?;
    configure(&conn)?;
    Ok(conn)
}

/// Applies WAL, `busy_timeout`, and foreign-key enforcement.
fn configure(conn: &Connection) -> CommenterCatResult<()> {
    conn.busy_timeout(BUSY_TIMEOUT)
        .map_err(|e| CommenterCatError::storage("setting busy_timeout").caused_by(e))?;
    // WAL is a no-op ("memory") on in-memory databases, which is fine.
    conn.pragma_update(None, "journal_mode", "WAL")
        .map_err(|e| CommenterCatError::storage("enabling WAL").caused_by(e))?;
    conn.pragma_update(None, "foreign_keys", "ON")
        .map_err(|e| CommenterCatError::storage("enabling foreign_keys").caused_by(e))?;
    Ok(())
}
