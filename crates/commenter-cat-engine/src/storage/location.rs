...
///
/// # Errors
/// Returns [`CommenterCatError::Storage`] if the directory cannot be created.
pub fn ensure_cache_dir(repo_root: &Path) -> CommenterCatResult<PathBuf> {
    let dir = cache_dir(repo_root);
    fs::create_dir_all(&dir).map_err(|e| {
        CommenterCatError::storage(format!("creating cache dir {}", dir.display())).caused_by(e)
    })?;
    Ok(dir)
}
