...
use commenter_cat_core::version::INDEX_DB_SCHEMA_VERSION;
...
impl IndexDb {
    ...
    fn init(conn: Connection) -> CommenterCatResult<Self> {
        conn.execute_batch(INDEX_SCHEMA)
            .map_err(|e| CommenterCatError::storage("creating index.db schema").caused_by(e))?;
        ...
        .map_err(|e| {
            CommenterCatError::storage("recording index.db schema version").caused_by(e)
        })?;
        Ok(Self { conn })
    }
    ...
    ///
    /// # Errors
    /// Returns [`CommenterCatError::Storage`] on a read error or malformed value.
    pub fn schema_version(&self) -> CommenterCatResult<u32> {
        let raw = self
            .meta(META_SCHEMA_VERSION)?
            .ok_or_else(|| CommenterCatError::storage("index.db missing schema version"))?;
        raw.parse().map_err(|e| {
            CommenterCatError::storage(format!("invalid index.db schema version {raw:?}"))
                .caused_by(e)
        })
    }
    ...
    pub fn set_meta(&self, key: &str, value: &str) -> CommenterCatResult<()> {
        self.conn
            .execute(
            ...
            .map_err(|e| {
                CommenterCatError::storage(format!("writing meta {key:?}")).caused_by(e)
            })?;
        Ok(())
    }
    ...
    ///
    /// # Errors
    /// Returns [`CommenterCatError::Storage`] on serialization or write failure.
    pub fn insert_comment(&self, comment: &Comment) -> CommenterCatResult<i64> {
        let record_json = serde_json::to_string(comment)
            .map_err(|e| CommenterCatError::storage("serializing comment").caused_by(e))?;
        self.conn
            .execute(
                ...
                params![
                    ...
                    comment
                        .bound_symbol
                        .as_ref()
                        .map(commenter_cat_core::BoundSymbol::as_str),
                    comment.is_rot_candidate,
                    comment.cosmetic_fingerprint,
                    record_json,
                ],
            )
            .map_err(|e| CommenterCatError::storage("inserting comment").caused_by(e))?;
    ...
    ///
    /// # Errors
    /// Returns [`CommenterCatError::Storage`] on a read or deserialization error.
    pub fn comment(&self, id: i64) -> CommenterCatResult<Option<Comment>> {
        let json: Option<String> = self
            ...
            .optional()
            .map_err(|e| CommenterCatError::storage("reading comment").caused_by(e))?;
        match json {
            Some(raw) => Ok(Some(serde_json::from_str(&raw).map_err(|e| {
                CommenterCatError::storage("deserializing comment").caused_by(e)
            })?)),
            None => Ok(None),
        }
    }
    ...
    /// # Errors
    /// Returns [`CommenterCatError::Storage`] on a read error.
    pub fn comment_texts(&self) -> CommenterCatResult<Vec<(i64, String)>> {
        let mut stmt = self
            .conn
            .prepare("SELECT id, raw_text FROM comments ORDER BY id")
            .map_err(|e| CommenterCatError::storage("preparing comment scan").caused_by(e))?;
        let rows = stmt
            .query_map([], |row| {
                Ok((row.get::<_, i64>(0)?, row.get::<_, String>(1)?))
            })
            .map_err(|e| CommenterCatError::storage("scanning comments").caused_by(e))?;
        let mut out = Vec::new();
        for row in rows {
            out.push(
                row.map_err(|e| CommenterCatError::storage("reading comment row").caused_by(e))?,
            );
        }
        Ok(out)
    }
    ...
    /// # Errors
    /// Returns [`CommenterCatError::Storage`] on serialization or write failure.
    pub fn insert_finding(&self, comment_id: i64, finding: &Finding) -> CommenterCatResult<()> {
        let finding_json = serde_json::to_string(finding)
            .map_err(|e| CommenterCatError::storage("serializing finding").caused_by(e))?;
        self.conn
            .execute(
            ...
            .map_err(|e| CommenterCatError::storage("inserting finding").caused_by(e))?;
        Ok(())
    }
    ...
    /// # Errors
    /// Returns [`CommenterCatError::Storage`] on a read or deserialization error.
    pub fn findings_for_comment(&self, comment_id: i64) -> CommenterCatResult<Vec<Finding>> {
        let mut stmt = self
            .conn
            .prepare("SELECT finding_json FROM findings WHERE comment_id = ?1 ORDER BY id")
            .map_err(|e| CommenterCatError::storage("preparing findings query").caused_by(e))?;
        let rows = stmt
            .query_map(params![comment_id], |row| row.get::<_, String>(0))
            .map_err(|e| CommenterCatError::storage("querying findings").caused_by(e))?;
        let mut out = Vec::new();
        for row in rows {
            let raw =
                row.map_err(|e| CommenterCatError::storage("reading finding row").caused_by(e))?;
            out.push(
                serde_json::from_str(&raw).map_err(|e| {
                    CommenterCatError::storage("deserializing finding").caused_by(e)
                })?,
            );
        }
        Ok(out)
