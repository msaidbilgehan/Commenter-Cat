...
use commenter_cat_core::identity::CommentIdentity;
...
///
/// # Errors
/// Returns [`CommenterCatError::Identity`] on a write error.
pub fn store(
    index: &IndexDb,
    comment_id: i64,
    ...
    index
        ...
        .map_err(|e| CommenterCatError::identity("storing identity").caused_by(e))?;
    Ok(())
}
...
///
/// # Errors
/// Returns [`CommenterCatError::Identity`] on a read error or an unrecognized stored kind.
pub fn get(index: &IndexDb, comment_id: i64) -> CommenterCatResult<Option<CommentIdentity>> {
    let row = index
        .conn()
        .query_row(
        ...
        .optional()
        .map_err(|e| CommenterCatError::identity("reading identity").caused_by(e))?;

    match row {
        None => Ok(None),
        Some((bound_symbol, kind_token, cosmetic_fingerprint, ordinal)) => {
            let kind = CommentKind::from_token(&kind_token).ok_or_else(|| {
                CommenterCatError::identity(format!("unknown stored kind {kind_token:?}"))
            })?;
...
///
/// # Errors
/// Returns [`CommenterCatError::Identity`] on a read error.
pub fn by_base(
    index: &IndexDb,
    bound_symbol: Option<&str>,
    kind: CommentKind,
    cosmetic_fingerprint: &str,
) -> CommenterCatResult<Vec<(i64, u32)>> {
    let mut stmt = index
        .conn()
        ...
        .map_err(|e| CommenterCatError::identity("preparing identity query").caused_by(e))?;
    let rows = stmt
        .query_map(
            params![bound_symbol, kind.as_str(), cosmetic_fingerprint],
            |row| Ok((row.get::<_, i64>(0)?, row.get::<_, u32>(1)?)),
        )
        .map_err(|e| CommenterCatError::identity("querying identities").caused_by(e))?;
    let mut out = Vec::new();
    for row in rows {
        out.push(
            row.map_err(|e| CommenterCatError::identity("reading identity row").caused_by(e))?,
        );
    }
    Ok(out)
}
