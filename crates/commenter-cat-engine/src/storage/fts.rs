...
pub fn create(conn: &Connection) -> CommenterCatResult<()> {
    conn.execute_batch("CREATE VIRTUAL TABLE IF NOT EXISTS comments_fts USING fts5(raw_text);")
        .map_err(|e| CommenterCatError::storage("creating FTS5 index").caused_by(e))
}
...
pub fn index(conn: &Connection, comment_id: i64, raw_text: &str) -> CommenterCatResult<()> {
    conn.execute(
        "INSERT INTO comments_fts(rowid, raw_text) VALUES (?1, ?2)",
        params![comment_id, raw_text],
    )
    .map_err(|e| CommenterCatError::storage("indexing comment for FTS").caused_by(e))?;
    Ok(())
}
...
pub fn search(conn: &Connection, query: &str, limit: usize) -> CommenterCatResult<Vec<i64>> {
    let mut stmt = conn
        .prepare(
            "SELECT rowid FROM comments_fts
             WHERE comments_fts MATCH ?1 ORDER BY rank LIMIT ?2",
        )
        .map_err(|e| CommenterCatError::storage("preparing FTS query").caused_by(e))?;
    let rows = stmt
        .query_map(params![query, limit as i64], |row| row.get::<_, i64>(0))
        .map_err(|e| CommenterCatError::storage("running FTS query").caused_by(e))?;
    let mut ids = Vec::new();
    for row in rows {
        ids.push(row.map_err(|e| CommenterCatError::storage("reading FTS row").caused_by(e))?);
    }
    Ok(ids)
}
