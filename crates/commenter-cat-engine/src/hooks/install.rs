//! `commenter-cat install-hooks` (Idea §7, §9).
//!
...
//! commenter-cat-managed directory — we never hand-edit `.git/hooks/` (so a user's existing
...
/// The commenter-cat-managed hooks directory (set as `core.hooksPath`). It lives under the
...
pub fn hook_script(event: HookEvent) -> String {
    ...
    // pass runs `commenter-cat` on only those. Every line is guarded so a failure (commenter-cat
    ...
    format!(
        "#!/bin/sh\n\
         ...
         if [ -n \"$changed\" ]; then\n\
         \x20 commenter-cat check $changed >/dev/null 2>&1 || true\n\
         fi\n\
         exit 0\n",
...
///
/// # Errors
/// Returns [`CommenterCatError::Config`] if the directory/scripts cannot be written or
/// `git config` fails.
pub fn install(repo_root: &Path) -> CommenterCatResult<InstallReport> {
    let dir = hooks_dir(repo_root);
    std::fs::create_dir_all(&dir).map_err(|e| {
        CommenterCatError::config(format!("creating hooks dir {}", dir.display())).caused_by(e)
    })?;
    ...
    for event in HookEvent::ALL {
        ...
        std::fs::write(&path, hook_script(event)).map_err(|e| {
            CommenterCatError::config(format!("writing hook {}", path.display())).caused_by(e)
        })?;
...
/// `.git/hooks/`).
fn set_hooks_path(repo_root: &Path, dir: &Path) -> CommenterCatResult<()> {
    let status = Command::new("git")
        ...
        .status()
        .map_err(|e| {
            CommenterCatError::config("spawning `git config core.hooksPath`").caused_by(e)
        })?;
    if !status.success() {
        return Err(CommenterCatError::config(
...
fn make_executable(path: &Path) -> CommenterCatResult<()> {
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        let mut perms = std::fs::metadata(path)
            .map_err(|e| CommenterCatError::config("reading hook permissions").caused_by(e))?
            .permissions();
        perms.set_mode(0o755);
        std::fs::set_permissions(path, perms)
            .map_err(|e| CommenterCatError::config("setting hook executable bit").caused_by(e))?;
    }
    #[cfg(not(unix))]
    let _ = path;
    Ok(())
...
mod tests {
    ...
    fn test_script_warms_only_changed_files_and_is_non_fatal() {
        ...
        assert!(script.contains("commenter-cat check $changed"));
