...
///
/// # Errors
/// Returns [`CommenterCatError::Extract`] if the file cannot be read, is not valid UTF-8,
/// or the grammar fails to parse it.
pub fn extract_file(walked: &WalkedFile, root: &Path) -> CommenterCatResult<Vec<Comment>> {
    let bytes = fs::read(&walked.path).map_err(|e| {
        CommenterCatError::extract(format!("reading {}", walked.path.display())).caused_by(e)
    })?;
    let source = String::from_utf8(bytes).map_err(|e| {
        CommenterCatError::extract(format!("{} is not valid UTF-8", walked.path.display()))
            .caused_by(e)
    })?;
...
///
/// # Errors
/// Returns [`CommenterCatError::Extract`] if the grammar cannot be set or parsing yields
/// no tree.
pub fn extract_source(
    ...
    parser.set_language(&grammar).map_err(|e| {
        CommenterCatError::extract(format!("setting {language} grammar failed: {e}"))
    })?;
    let tree = parser.parse(source.as_bytes(), None).ok_or_else(|| {
        CommenterCatError::extract(format!("tree-sitter produced no tree for {repo_path}"))
    })?;
