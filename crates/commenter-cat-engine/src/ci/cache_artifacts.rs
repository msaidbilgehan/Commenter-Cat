...
//!
//! The whole point: a Commenter-Cat upgrade should **reuse expensive provider results**. So
//! the two artifacts are keyed differently:
//!
//! * `inputs.db` — keyed on provider versions + config hashes + the inputs
//!   schema. It **excludes** `commenter_cat_ruleset_version`, so bumping Commenter-Cat does not
//!   invalidate it: the provider outputs are reused.
//! * `index.db` — keyed on the *full* comparability key (inputs material **plus**
//!   `commenter_cat_ruleset_version` + the index schema). A Commenter-Cat upgrade misses, and the index
//!   is **re-derived from `inputs.db`** — never cold-scanned.

use commenter_cat_core::version::{
    COMMENTER_CAT_RULESET_VERSION, INDEX_DB_SCHEMA_VERSION, INPUTS_DB_SCHEMA_VERSION,
};
...
/// **without** `commenter_cat_ruleset_version`, so a Commenter-Cat upgrade reuses provider results.
#[must_use]
...
/// `commenter_cat_ruleset_version` + index schema), so a Commenter-Cat upgrade misses and the index is
/// re-derived from `inputs.db`.
#[must_use]
pub fn index_cache_key(
    providers: &[ProviderFingerprint],
    commenter_cat_ruleset_version: u32,
) -> String {
    let material = format!(
        "{};commenter_cat_ruleset={commenter_cat_ruleset_version};index_schema={INDEX_DB_SCHEMA_VERSION}",
...
pub enum RestorePlan {
    ...
    /// (no provider re-runs, no cold scan). The Commenter-Cat-upgrade fast path.
    ReDeriveFromInputs,
...
/// The current Commenter-Cat ruleset version (for the index key at run time).
#[must_use]
...
mod tests {
    ...
    #[test]
    fn test_commenter_cat_upgrade_reuses_inputs_but_remisses_index() {
        let providers = providers();
        let v = current_ruleset_version();

        // A Commenter-Cat upgrade = a bumped ruleset version, same providers.
        let inputs_before = inputs_cache_key(&providers);
        let inputs_after = inputs_cache_key(&providers);
        assert_eq!(
            inputs_before, inputs_after,
            "inputs key ignores the Commenter-Cat version"
        );
        ...
        assert_ne!(
            ...
            "index key tracks the Commenter-Cat version"
        );
    }
    ...
    fn test_restore_plan() {
        ...
        // The Commenter-Cat-upgrade fast path: inputs hit, index miss → re-derive.
