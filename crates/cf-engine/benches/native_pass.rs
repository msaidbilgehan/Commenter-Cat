...
//! comment→code mapping are the hot path of `cf check`. A regression here shows
//! up as a wall-clock change against this baseline (the `cf check --stats`
//! budget, §6).
...
use cf_engine::extract::coalesce::coalesce;
use cf_engine::extract::extract_source;
use cf_engine::map::map_comments;
