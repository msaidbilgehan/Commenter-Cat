...
//!
//! Restore the two cache layers (so a CF upgrade reuses provider results), diff
...
pub struct CiReport {
    /// Pass / Fail / Degraded.
    pub verdict: Verdict,
    /// How the caches were restored (the CF-upgrade fast path is observable here).
...
///
/// # Errors
/// Returns [`cf_core::CfError`] if a structured artifact fails to render.
pub fn run(
    comments: &[Comment],
...
mod tests {
    ...
    #[test]
    fn test_cf_upgrade_reuses_inputs_and_rederives_index() {
        // inputs hit, index miss (the CF-upgrade case) → re-derive, no cold scan.
        let report = run(
            &[comment(vec![])],
