...
use cf_core::error::{cause_chain, CfError, CfResult};
...
#[tool_handler]
impl ServerHandler for CfServer {
    /// Identifies the server as `cf` (not the SDK) and advertises the tools
    /// capability. Providing `get_info` suppresses the macro's default.
    fn get_info(&self) -> ServerInfo {
        let mut info = ServerInfo::new(ServerCapabilities::builder().enable_tools().build());
        info.server_info.name = "cf".to_owned();
...
/// for the arbitrary JSON shape). A `CfError` maps to an MCP error carrying the
/// cause chain.
fn dispatch(name: &str, args: Value) -> Result<CallToolResult, ErrorData> {
    let to_mcp_err = |e: CfError| ErrorData::internal_error(cause_chain(&e), None);
    let value = McpSurface::call(name, &args).map_err(to_mcp_err)?;
    let text = serde_json::to_string(&value)
        .map_err(|e| to_mcp_err(CfError::render("serializing MCP tool result").caused_by(e)))?;
...
///
/// # Errors
/// Returns [`CfError::Render`] if the runtime or transport fails.
pub fn serve_stdio() -> CfResult<()> {
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .map_err(|e| CfError::render("building the MCP async runtime").caused_by(e))?;
    runtime.block_on(serve())
}

/// The async stdio serve loop.
async fn serve() -> CfResult<()> {
    use rmcp::transport::stdio;
    use rmcp::ServiceExt;

    let service = CfServer::new()
        .serve(stdio())
        .await
        .map_err(|e| CfError::render(format!("starting the MCP server: {e}")))?;
    service
        .waiting()
        .await
        .map_err(|e| CfError::render(format!("MCP server run error: {e}")))?;
    Ok(())
}
...
mod tests {
    ...
    fn test_get_info_enables_the_tools_capability() {
        let info = CfServer::new().get_info();
        assert!(
