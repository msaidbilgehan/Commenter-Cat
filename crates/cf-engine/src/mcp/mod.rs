...
impl McpSurface {
    ...
    ///
    /// # Errors
    /// Returns [`CfError`] for an unknown tool, a dispatch failure, or a verb
    /// that requires the persisted index before it has been built.
    pub fn call(name: &str, args: &Value) -> CfResult<Value> {
        let tool = McpTool::from_name(name)
            .ok_or_else(|| CfError::config(format!("unknown MCP tool {name:?}")))?;
        match tool {
...
fn root_from(args: &Value) -> CfResult<PathBuf> {
...
fn check(args: &Value) -> CfResult<Value> {
...
fn candidates(args: &Value) -> CfResult<Value> {
    let root = root_from(args)?;
    let limit = args
        .get("limit")
...
fn query(args: &Value) -> CfResult<Value> {
...
fn context(args: &Value) -> CfResult<Value> {
    let session = Session::open(&root_from(args)?)?;
...
fn apply_edit(args: &Value) -> CfResult<Value> {
    let session = Session::open(&root_from(args)?)?;
    let comment = resolve(&session, args)?;
    let new_text = args
        .get("new_text")
        .and_then(Value::as_str)
        .ok_or_else(|| CfError::config("apply_edit requires `new_text`"))?;
...
fn remove(args: &Value) -> CfResult<Value> {
    let session = Session::open(&root_from(args)?)?;
    let comment = resolve(&session, args)?;
    let allow_significant = args
        .get("allow_significant")
...
fn resolve(session: &Session, args: &Value) -> CfResult<Comment> {
    let id = args
        .get("comment_id")
        .and_then(Value::as_str)
        .and_then(|s| s.parse::<i64>().ok())
        .ok_or_else(|| CfError::config("a numeric `comment_id` (from `query`) is required"))?;
    session
        .comment(id)?
        .ok_or_else(|| CfError::config(format!("no comment {id} in the index")))
}
