...
///
/// # Errors
/// Returns [`CfError::Git`] if a commit or tree cannot be read, or the diff fails.
pub fn changed_between(repo: &Repo, old: ObjectId, new: ObjectId) -> CfResult<Vec<String>> {
    let inner = repo.inner();
    let old_commit = inner
        .find_commit(old)
        .map_err(|e| CfError::git(format!("reading commit {old}")).caused_by(e))?;
    let new_commit = inner
        .find_commit(new)
        .map_err(|e| CfError::git(format!("reading commit {new}")).caused_by(e))?;
    let old_tree = old_commit
        .tree()
        .map_err(|e| CfError::git(format!("reading tree of {old}")).caused_by(e))?;
    let new_tree = new_commit
        .tree()
        .map_err(|e| CfError::git(format!("reading tree of {new}")).caused_by(e))?;

    let changes = inner
        .diff_tree_to_tree(Some(&old_tree), Some(&new_tree), None)
        .map_err(|e| CfError::git("diffing commit trees").caused_by(e))?;
...
/// # Errors
/// Returns [`CfError::Git`] if a commit or tree cannot be read, or the diff fails.
pub fn changed_in_commit(repo: &Repo, commit: ObjectId) -> CfResult<Vec<String>> {
    let inner = repo.inner();
    let commit_obj = inner
        .find_commit(commit)
        .map_err(|e| CfError::git(format!("reading commit {commit}")).caused_by(e))?;
    let new_tree = commit_obj
        .tree()
        .map_err(|e| CfError::git(format!("reading tree of {commit}")).caused_by(e))?;
    ...
    let old_tree = match parent {
        Some(parent_id) => {
            let parent_commit = inner
                ...
                .map_err(|e| CfError::git("reading parent commit").caused_by(e))?;
            Some(
                parent_commit
                    .tree()
                    .map_err(|e| CfError::git("reading parent tree").caused_by(e))?,
            )
        }
        None => None,
    };
    ...
    let changes = inner
        ...
        .map_err(|e| CfError::git("diffing against parent").caused_by(e))?;
    Ok(collect_paths(&changes))
}
