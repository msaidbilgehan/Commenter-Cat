...
pub fn create(conn: &Connection, dimensions: usize) -> CfResult<()> {
    let ddl = format!(
        "CREATE VIRTUAL TABLE IF NOT EXISTS comments_vec USING vec0(embedding float[{dimensions}]);"
    );
    conn.execute_batch(&ddl)
        .map_err(|e| CfError::storage("creating vec0 index").caused_by(e))
}
...
pub fn index(conn: &Connection, comment_id: i64, vector: &[f32]) -> CfResult<()> {
    conn.execute(
        "INSERT INTO comments_vec(rowid, embedding) VALUES (?1, ?2)",
        params![comment_id, vector_to_blob(vector)],
    )
    .map_err(|e| CfError::storage("indexing comment vector").caused_by(e))?;
    Ok(())
}
...
pub fn knn(conn: &Connection, query: &[f32], k: usize) -> CfResult<Vec<i64>> {
    let mut stmt = conn
        .prepare(
            "SELECT rowid FROM comments_vec
             WHERE embedding MATCH ?1 AND k = ?2 ORDER BY distance",
        )
        .map_err(|e| CfError::storage("preparing kNN query").caused_by(e))?;
    let rows = stmt
        .query_map(params![vector_to_blob(query), k as i64], |row| {
            row.get::<_, i64>(0)
        })
        .map_err(|e| CfError::storage("running kNN query").caused_by(e))?;
    let mut ids = Vec::new();
    for row in rows {
        ids.push(row.map_err(|e| CfError::storage("reading kNN row").caused_by(e))?);
    }
    Ok(ids)
}
