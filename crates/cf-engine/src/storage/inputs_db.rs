...
//! ONNX inference — so it is keyed by **content**, never by CF's layout:
//!
//! * provider results by `(content_hash, provider, provider_version)`, and
//! * embedding vectors by `(content_hash, model_version)`.
//!
//! Because the keys are content hashes, a CF schema change never invalidates it;
...
use cf_core::version::INPUTS_DB_SCHEMA_VERSION;
...
impl InputsDb {
    ...
    fn init(conn: Connection) -> CfResult<Self> {
        conn.execute_batch(SCHEMA)
            .map_err(|e| CfError::storage("creating inputs.db schema").caused_by(e))?;
        ...
        .map_err(|e| CfError::storage("recording inputs.db schema version").caused_by(e))?;
        Ok(Self { conn })
    }
    ...
    pub fn schema_version(&self) -> CfResult<u32> {
        let raw = self
            ...
            .ok_or_else(|| CfError::storage("inputs.db missing schema version"))?;
        raw.parse().map_err(|e| {
            CfError::storage(format!("invalid inputs.db schema version {raw:?}")).caused_by(e)
        })
    }
    ...
    pub fn store_provider_result(
        ...
        self.conn
            ...
            .map_err(|e| CfError::storage(format!("caching {provider} result")).caused_by(e))?;
        Ok(())
    }
    ...
    pub fn provider_result(
        ...
        provider: &str,
        provider_version: &str,
    ) -> CfResult<Option<CachedProviderResult>> {
        self.conn
            .query_row(
            ...
            .optional()
            .map_err(|e| CfError::storage(format!("reading {provider} cache")).caused_by(e))
    }
    ...
    pub fn store_embedding(
        ...
        self.conn
            ...
            .map_err(|e| CfError::storage("caching embedding").caused_by(e))?;
        Ok(())
    }
    ...
    /// # Errors
    /// Returns [`CfError::Storage`] on a read error.
    pub fn embedding(&self, content_hash: &str, model_version: &str) -> CfResult<Option<Vec<f32>>> {
        let blob: Option<Vec<u8>> = self
            ...
            .optional()
            .map_err(|e| CfError::storage("reading embedding").caused_by(e))?;
