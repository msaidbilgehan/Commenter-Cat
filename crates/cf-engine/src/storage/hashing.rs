...
pub fn hash_file(path: &Path) -> CfResult<String> {
    let bytes = fs::read(path)
        .map_err(|e| CfError::storage(format!("hashing {}", path.display())).caused_by(e))?;
    Ok(sha256_hex(&bytes))
}
...
impl FileStat {
    ...
    ///
    /// # Errors
    /// Returns [`CfError::Storage`] if the file cannot be stat-ed.
    pub fn of(path: &Path) -> CfResult<Self> {
        let meta = fs::metadata(path)
            .map_err(|e| CfError::storage(format!("stat {}", path.display())).caused_by(e))?;
        Ok(Self {
