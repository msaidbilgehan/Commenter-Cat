...
///
/// # Errors
/// Returns [`CfError::Map`] if the grammar cannot be set or parsing fails.
pub fn map_comments(
    source: &str,
    language: Language,
    grammar_path: &Path,
    ...
    parser
        ...
        .map_err(|e| CfError::map(format!("setting {language} grammar failed: {e}")))?;
    let tree = parser
        .parse(source.as_bytes(), None)
        .ok_or_else(|| CfError::map("tree-sitter produced no tree for mapping"))?;
