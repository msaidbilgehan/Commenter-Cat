...
///
/// # Errors
/// Returns [`CfError::Apply`] if the range is out of bounds, a grammar fails to
/// parse, a Python docstring is broken into code, or any code node changes.
pub fn apply_comment_edit(source: &str, comment: &Comment, new_text: &str) -> CfResult<String> {
    ...
    let new_exclude = if is_python_docstring {
        Some(python_docstring_range(&new_tree, start).ok_or_else(|| {
            CfError::apply("edit would break the docstring into code (parse-invariance)")
        })?)
    } else {
        None
    };
...
///
/// # Errors
/// Returns [`CfError::Apply`] if the position is invalid, parsing fails, or the
...
fn parse(source: &str, language: Language, path: &Path) -> CfResult<Tree> {
    let grammar = grammars::grammar_for(language, path);
    let mut parser = Parser::new();
    parser
        .set_language(&grammar)
        .map_err(|e| CfError::apply(format!("setting {language} grammar failed: {e}")))?;
    parser
        .parse(source.as_bytes(), None)
        .ok_or_else(|| CfError::apply("tree-sitter produced no tree for the edit"))
}
