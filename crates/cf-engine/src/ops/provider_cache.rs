...
//!   `cf_scope` universe for a project-scoped tool (gitleaks scans the whole
//!   tree, but cf's *retained* findings depend only on universe content, so that
...
/// Provider-cache outcome for one `cf check`, surfaced by `--stats` (Idea §6).
...
/// The provider-result cache for one `cf check` run.
///
...
/// The `cf_scope` tree hash — over the universe (every non-ignored file). `None`
