...
//! `cf:disable` that no longer suppresses anything is reported).
//!
...
/// Whether a matching `cf:enable` closes `disable`'s region before `finding_line`.
...
mod tests {
    ...
    fn test_disable_line_and_next_line_scopes() {
        ...
        let on_line = directives::parse("# cf:disable-line=D417", 5).unwrap();
        assert_eq!(suppress(&findings, &[on_line]).decisions.len(), 1);

        let next_line = directives::parse("# cf:disable-next-line=D417", 4).unwrap();
        assert_eq!(suppress(&findings, &[next_line]).decisions.len(), 1);

        let wrong_line = directives::parse("# cf:disable-line=D417", 9).unwrap();
    ...
    fn test_category_and_origin_targets() {
        let findings = [finding("D417", Category::DocDrift, 5)];
        let by_category = directives::parse("# cf:disable-line=doc_drift", 5).unwrap();
        assert_eq!(suppress(&findings, &[by_category]).decisions.len(), 1);
        let by_origin = directives::parse("# cf:disable-line=ruff", 5).unwrap();
        assert_eq!(suppress(&findings, &[by_origin]).decisions.len(), 1);
        let by_other = directives::parse("# cf:disable-line=eslint", 5).unwrap();
    ...
    fn test_disable_enable_region() {
        ...
        let directives = [
            directives::parse("# cf:disable=D417", 1).unwrap(),
            directives::parse("# cf:enable=D417", 6).unwrap(),
        ];
        ...
        assert_eq!(
            ...
            vec![SuppressionDecision {
                finding_index: 0,
                suppressed_by: "cf:disable=D417".to_owned()
            }]
        );
    }
    ...
    fn test_disable_file_and_unused_reported() {
        let findings = [finding("D417", Category::DocDrift, 5)];
        let directives = [
            directives::parse("# cf:disable-file=D417", 1).unwrap(),
            directives::parse("# cf:disable-line=D100", 2).unwrap(), // matches nothing
        ];
        let outcome = suppress(&findings, &directives);
