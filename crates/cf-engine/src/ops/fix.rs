//! `cf fix` / `cf tighten` (Idea §5; task 7.6).
//!
//! Two fix sources, by design (Idea §5):
//!
//! * **Provider autofixes** are *delegated* to each tool's own `--fix`
//!   (`ruff --fix`, `eslint --fix`) where `capabilities.supports_fix` — CF never
...
use cf_core::finding::{Finding, Fix};
...
pub enum FixRoute {
    /// Delegate to the provider's own `--fix` (CF never hand-applies).
    ProviderAutofix,
...
mod tests {
    ...
    use cf_core::finding::{Category, FindingTarget, Origin, Range};
