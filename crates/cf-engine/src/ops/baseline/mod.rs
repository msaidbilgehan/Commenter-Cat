...
//! suppression pass. `cf baseline accept` snapshots current findings; `cf
...
/// (`cf baseline accept`). Deterministic: sorted + de-duplicated.
#[must_use]
...
/// (`cf baseline prune`). Reasons/dates on surviving entries are preserved.
#[must_use]
...
pub fn load(path: &Path) -> CfResult<Baseline> {
    let text = std::fs::read_to_string(path).map_err(|e| {
        CfError::config(format!("reading baseline {}", path.display())).caused_by(e)
    })?;
    let baseline: Baseline = toml::from_str(&text).map_err(|e| {
        CfError::config(format!("parsing baseline {}", path.display())).caused_by(e)
    })?;
    if baseline.version > BASELINE_VERSION {
        return Err(CfError::config(format!(
            "baseline at {} declares version {}, but this cf supports up to {BASELINE_VERSION}; \
             run `cf baseline migrate`",
            path.display(),
            baseline.version
        )));
    }
...
pub fn save(baseline: &Baseline, path: &Path) -> CfResult<()> {
    let mut canonical = baseline.clone();
    canonical.canonicalize();
    let text = toml::to_string(&canonical)
        .map_err(|e| CfError::config("serializing baseline").caused_by(e))?;
    std::fs::write(path, text).map_err(|e| {
        CfError::config(format!("writing baseline {}", path.display())).caused_by(e)
    })?;
    Ok(())
}
