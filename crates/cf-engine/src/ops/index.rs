...
//!
//! `cf check` fuses the unified records in memory; [`persist`] writes them into
...
/// keeps `cf check`/`cf query` offline + instant and aids reproducibility (Idea
...
///
/// # Errors
/// Returns [`CfError::Storage`] on any cache write failure.
pub fn persist<E: Embedder>(repo_root: &Path, comments: &[Comment], embedder: &E) -> CfResult<()> {
    ...
    if index_path.exists() {
        std::fs::remove_file(&index_path).map_err(|e| {
            CfError::storage(format!("removing stale index {}", index_path.display())).caused_by(e)
        })?;
    }
...
impl Session {
    ...
    ///
    /// # Errors
    /// Returns [`CfError::Storage`] if `index.db` is absent (run `cf check`) or
    /// cannot be opened.
    pub fn open(repo_root: &Path) -> CfResult<Session> {
        let index_path = location::index_db_path(repo_root);
        if !index_path.exists() {
            return Err(CfError::storage(
                "no index found — run `cf check` to build it before query/context/apply",
            ));
        }
        Ok(Session {
    ...
    ///
    /// # Errors
    /// Returns [`cf_core::CfError`] on a search or read error.
    pub fn query(&self, text: &str, limit: usize) -> CfResult<Vec<(i64, Comment)>> {
    ...
    ///
    /// # Errors
    /// Returns [`cf_core::CfError`] on a read error.
    ...
    ///
    /// # Errors
    /// Returns [`CfError::Storage`] if the file cannot be read.
    pub fn read_source(&self, comment: &Comment) -> CfResult<String> {
        let path = self.file_path(comment);
        std::fs::read_to_string(&path)
            .map_err(|e| CfError::storage(format!("reading {}", path.display())).caused_by(e))
    }

    /// Writes new source back to a comment's file (after a safe edit).
    ///
    /// # Errors
    /// Returns [`CfError::Storage`] if the file cannot be written.
    pub fn write_source(&self, comment: &Comment, source: &str) -> CfResult<()> {
        let path = self.file_path(comment);
        std::fs::write(&path, source)
            .map_err(|e| CfError::storage(format!("writing {}", path.display())).caused_by(e))
    }
