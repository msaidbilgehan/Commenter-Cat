...
///
/// # Errors
/// Returns [`CfError::Provider`] if any JSONPath is invalid.
pub fn extract(provider: &str, output: &Value, specs: &[FindingSpec]) -> CfResult<Vec<RawFinding>> {
    let mut raw = Vec::new();
    for spec in specs {
...
fn compile(provider: &str, path: &str) -> CfResult<JsonPath> {
    JsonPath::parse(path)
        .map_err(|e| CfError::provider(provider, format!("invalid JSONPath {path:?}: {e}")))
}

/// All values matched by `path` within `root`.
fn query_all<'a>(provider: &str, path: &str, root: &'a Value) -> CfResult<Vec<&'a Value>> {
...
fn query_string(provider: &str, path: &str, node: &Value) -> CfResult<Option<String>> {
...
fn query_u32(provider: &str, path: &str, node: &Value) -> CfResult<Option<u32>> {
    Ok(compile(provider, path)?
        .query(node)
        .first()
