//! `cf doctor` — version + config validation (Idea §5; task 6.7).
//!
//! The promise is "same source + same config ⇒ same findings". `cf doctor`
...
//! (Idea §5). The full comparability key is `(cf_ruleset_version,
//! provider_version, config_hash)`.

use serde::{Deserialize, Serialize};

use cf_core::version::ComparabilityKey;
