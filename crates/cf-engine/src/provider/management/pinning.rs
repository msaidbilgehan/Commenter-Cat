...
//! are fetched/cached at `~/.cache/cf/providers/` (single-binary tools cleanly;
...
/// The provider cache root (`$XDG_CACHE_HOME/cf/providers` or
/// `$HOME/.cache/cf/providers`), if determinable from the environment.
#[must_use]
pub fn provider_cache_dir(get_env: impl Fn(&str) -> Option<String>) -> Option<PathBuf> {
    ...
    if let Some(xdg) = value("XDG_CACHE_HOME") {
        return Some(PathBuf::from(xdg).join("cf").join("providers"));
    }
    value("HOME").map(|home| {
        PathBuf::from(home)
            .join(".cache")
...
mod tests {
    ...
    fn test_cache_dir_prefers_xdg_then_home() {
        ...
        assert!(cache.ends_with("cf/providers") && cache.starts_with("/c"));
        let home = provider_cache_dir(env(&[("HOME", "/home/u")])).unwrap();
        assert!(home.starts_with("/home/u/.cache/cf/providers"));
        assert!(provider_cache_dir(env(&[])).is_none());
    }

    #[test]
    fn test_provider_path_layout() {
        let cache = PathBuf::from("/c/cf/providers");
        let path = provider_path(&cache, "ruff", &PinnedVersion::new("0.14.2"));
        assert_eq!(path, PathBuf::from("/c/cf/providers/ruff/0.14.2"));
    }
}
