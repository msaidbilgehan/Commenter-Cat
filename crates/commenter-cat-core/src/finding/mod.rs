...
pub enum Origin {
    ...
    /// `gitleaks` (secrets).
    Gitleaks,
    /// A Commenter-Cat-native check (blame-skew, marker triage).
    Native,
...
impl Origin {
    ...
    /// Whether this is the Commenter-Cat-native origin.
    #[must_use]
...
pub enum Fix {
    /// The provider can autofix via its own `--fix` (Commenter-Cat delegates, Idea §5).
    ProviderAutofix,
...
impl Range {
    ...
    /// Providers report a line+column that Commenter-Cat converts to a single byte offset, so
...
pub struct Finding {
    ...
    /// The rule id adopted into Commenter-Cat's vocabulary (e.g. `"D417"`).
