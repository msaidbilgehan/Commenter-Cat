...
//!
//! [`CommenterCatError`] is the single error type every Commenter-Cat crate returns. Its
...
//! optional cause chain via `#[source]` ([`CommenterCatError::caused_by`]).
//!
//! ## Why explicit wrapping, not blanket `#[from]`
//!
//! Infrastructure errors (`rusqlite::Error`, subprocess failures, `gix` errors)
//! are **translated at the `commenter-cat-engine` adapter boundary** into a `CommenterCatError` with
...
impl CommenterCatError {
    ...
    ///
    /// ```
    /// # use commenter_cat_core::error::CommenterCatError;
    /// let io = std::io::Error::new(std::io::ErrorKind::NotFound, "missing");
    /// let err = CommenterCatError::config("reading commenter-cat.toml").caused_by(io);
    /// assert!(std::error::Error::source(&err).is_some());
    /// ```
    #[must_use]
    pub fn caused_by(mut self, source: impl Into<BoxError>) -> Self {
        let slot = match &mut self {
            CommenterCatError::Config { source, .. }
            | CommenterCatError::Walk { source, .. }
            | CommenterCatError::Extract { source, .. }
            | CommenterCatError::Map { source, .. }
            | CommenterCatError::Storage { source, .. }
            | CommenterCatError::Provider { source, .. }
            | CommenterCatError::Apply { source, .. }
            | CommenterCatError::Identity { source, .. }
            | CommenterCatError::Git { source, .. }
            | CommenterCatError::Render { source, .. } => source,
        };
    ...
    pub const fn subsystem(&self) -> &'static str {
        match self {
            CommenterCatError::Config { .. } => "config",
            CommenterCatError::Walk { .. } => "walk",
            CommenterCatError::Extract { .. } => "extract",
            CommenterCatError::Map { .. } => "map",
            CommenterCatError::Storage { .. } => "storage",
            CommenterCatError::Provider { .. } => "provider",
            CommenterCatError::Apply { .. } => "apply",
            CommenterCatError::Identity { .. } => "identity",
            CommenterCatError::Git { .. } => "git",
            CommenterCatError::Render { .. } => "render",
        }
    }
}
...
#[must_use]
pub fn cause_chain(err: &CommenterCatError) -> String {
    use std::error::Error;
...
mod tests {
    ...
    fn test_display_includes_subsystem_and_message() {
        let err = CommenterCatError::config("missing [scan] table");
    ...
    fn test_provider_variant_names_the_provider() {
        let err = CommenterCatError::provider("ruff", "spawn failed");
    ...
    fn test_caused_by_preserves_source_chain() {
        ...
        let err = CommenterCatError::storage("opening index.db").caused_by(io);
    ...
    fn test_cause_chain_walks_to_root() {
        ...
        let err = CommenterCatError::walk("scanning repo root").caused_by(io);
    ...
    fn test_no_source_when_unset() {
        let err = CommenterCatError::map("orphan comment");
    ...
    fn test_error_is_send_and_sync() {
        // CommenterCatError must cross thread boundaries (rayon native pass, async MCP).
        fn assert_send_sync<T: Send + Sync>() {}
        assert_send_sync::<CommenterCatError>();
    }
}
