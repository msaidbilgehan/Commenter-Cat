//! Config discovery: walk-up cascade, XDG global layer, and `COMMENTER_CAT_*` env overrides
//! (Idea §12, task 1.4).
//!
//! ## Precedence (highest wins)
//!
//! 1. `COMMENTER_CAT_*` environment variables.
...
use crate::error::{CommenterCatError, CommenterCatResult};
...
/// The environment-variable prefix for config overrides (`COMMENTER_CAT_<SECTION>_<FIELD>`).
pub const ENV_PREFIX: &str = "COMMENTER_CAT_";
...
pub fn discover(start_dir: &Path) -> CommenterCatResult<ResolvedConfig> {
...
) -> CommenterCatResult<ResolvedConfig> {
...
///
/// # Errors
/// Returns a [`CommenterCatError::Config`] if the file cannot be read, is not valid TOML,
/// or declares an unknown *future* [`CONFIG_VERSION`] (Idea §11).
pub fn load_file(path: &Path) -> CommenterCatResult<ConfigFile> {
    let text = std::fs::read_to_string(path).map_err(|e| {
        CommenterCatError::config(format!("reading config at {}", path.display())).caused_by(e)
    })?;
    let file: ConfigFile = toml::from_str(&text).map_err(|e| {
        CommenterCatError::config(format!("parsing config at {}", path.display())).caused_by(e)
    })?;
    validate_version(&file, path)?;
    Ok(file)
}

/// Builds a config layer from real `COMMENTER_CAT_*` environment variables.
///
/// # Errors
/// Returns a [`CommenterCatError::Config`] if any present override fails to parse.
pub fn from_env() -> CommenterCatResult<ConfigFile> {
...
fn validate_version(file: &ConfigFile, path: &Path) -> CommenterCatResult<()> {
    if let Some(version) = file.version {
        if version > CONFIG_VERSION {
            return Err(CommenterCatError::config(format!(
                "config at {} declares version {version}, but this commenter-cat supports up to {CONFIG_VERSION}; \
                 upgrade commenter-cat or lower the config version",
                path.display()
            )));
        }
    }
...
fn from_env_with(get_env: &impl Fn(&str) -> Option<String>) -> CommenterCatResult<ConfigFile> {
    let scan = ScanSection {
        respect_gitignore: env_bool(get_env, "COMMENTER_CAT_SCAN_RESPECT_GITIGNORE")?,
        languages: env_typed_list(
            get_env,
            "COMMENTER_CAT_SCAN_LANGUAGES",
            Language::from_token,
            &tokens_of(&Language::ALL.map(|l| l.as_str())),
        )?,
        extra_ignores: env_string_list(get_env, "COMMENTER_CAT_SCAN_EXTRA_IGNORES"),
        suppress_kinds: env_typed_list(
            get_env,
            "COMMENTER_CAT_SCAN_SUPPRESS_KINDS",
            CommentKind::from_token,
            &tokens_of(&CommentKind::ALL.map(|k| k.as_str())),
        )?,
    };
    let providers = ProvidersSection {
        python: env_value(get_env, "COMMENTER_CAT_PROVIDERS_PYTHON"),
        javascript: env_value(get_env, "COMMENTER_CAT_PROVIDERS_JAVASCRIPT"),
        typescript: env_value(get_env, "COMMENTER_CAT_PROVIDERS_TYPESCRIPT"),
        shell: env_value(get_env, "COMMENTER_CAT_PROVIDERS_SHELL"),
        secrets: env_value(get_env, "COMMENTER_CAT_PROVIDERS_SECRETS"),
        on_missing: env_enum(
            get_env,
            "COMMENTER_CAT_PROVIDERS_ON_MISSING",
            OnMissing::from_token,
            "warn, error",
        )?,
        docstring_convention: env_value(get_env, "COMMENTER_CAT_PROVIDERS_DOCSTRING_CONVENTION"),
    };
    let markers = MarkersSection {
        custom: env_string_list(get_env, "COMMENTER_CAT_MARKERS_CUSTOM"),
    ...
    let severity = SeveritySection {
        fail_on: env_enum(
            get_env,
            "COMMENTER_CAT_SEVERITY_FAIL_ON",
            Severity::from_token,
    ...
    let search = SearchSection {
        embeddings: env_enum(
            get_env,
            "COMMENTER_CAT_SEARCH_EMBEDDINGS",
    ...
    let output = OutputSection {
        default_format: env_enum(
            get_env,
            "COMMENTER_CAT_OUTPUT_DEFAULT_FORMAT",
...
fn env_bool(
    get_env: &impl Fn(&str) -> Option<String>,
    key: &str,
) -> CommenterCatResult<Option<bool>> {
    match env_value(get_env, key) {
        None => Ok(None),
        Some(raw) => match raw.to_ascii_lowercase().as_str() {
            "true" => Ok(Some(true)),
            "false" => Ok(Some(false)),
            other => Err(CommenterCatError::config(format!(
...
fn env_enum<T>(
    ...
    valid: &str,
) -> CommenterCatResult<Option<T>> {
    match env_value(get_env, key) {
        None => Ok(None),
        Some(raw) => parse(&raw).map(Some).ok_or_else(|| {
            CommenterCatError::config(format!("env {key}: unknown value '{raw}'; valid: {valid}"))
        }),
    }
}
...
fn env_typed_list<T>(
    ...
    parse: impl Fn(&str) -> Option<T>,
    valid: &str,
) -> CommenterCatResult<Option<Vec<T>>> {
...
mod tests {
    ...
    impl TempDir {
        fn new() -> Self {
            ...
            let path = std::env::temp_dir()
                .join(format!("commenter-cat-core-cfg-{}-{n}", std::process::id()));
    ...
    fn test_env_overrides_file() {
        ...
        let env = env_map(&[("COMMENTER_CAT_OUTPUT_DEFAULT_FORMAT", "jsonl")]);

        let cfg = discover_in(&tmp.path, &env, None).unwrap();
        assert_eq!(
            cfg.output.default_format,
            OutputFormat::Jsonl,
            "COMMENTER_CAT_* must outrank the file"
        );
    }
    ...
    fn test_unknown_future_version_is_hard_error() {
        ...
        assert!(
            message.contains("upgrade commenter-cat"),
    ...
    fn test_env_typed_list_and_bool_parse() {
        let env = env_map(&[
            ("COMMENTER_CAT_SCAN_LANGUAGES", "python, shell"),
            ("COMMENTER_CAT_SCAN_RESPECT_GITIGNORE", "FALSE"),
        ]);
    ...
    fn test_env_unknown_enum_value_errors_with_hint() {
        let env = env_map(&[("COMMENTER_CAT_OUTPUT_DEFAULT_FORMAT", "yaml")]);
